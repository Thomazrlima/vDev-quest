...
const auras: Aura[] = [
  { name: "Musgo", color: "#758b47", wash: "rgba(104,139,58,.72)" },
  { name: "Âmbar", color: "#d2a343", wash: "rgba(210,163,67,.72)" },
  { name: "Rubi", color: "#bb6551", wash: "rgba(187,101,81,.7)" },
  { name: "Maré", color: "#4a9ba1", wash: "rgba(74,155,161,.7)" },
  { name: "Éter", color: "#8875b8", wash: "rgba(136,117,184,.68)" },
  { name: "Cinzas", color: "#8b8476", wash: "rgba(139,132,118,.66)" }
];
...
export default function CharacterCreatorPage() {
  ...
  return <div className="creator-parchment min-h-screen"><GameNav /><main className="mx-auto max-w-[1480px] px-4 py-7 sm:px-6 sm:py-9"><div className="creator-scroll-heading"><span>✦</span><p>Oficina de aventuras</p><span>✦</span></div><div className="mt-2 grid gap-5 xl:grid-cols-[285px_minmax(360px,1fr)_350px] xl:items-stretch">
    <section className="creator-ornate-panel p-4 sm:p-5" aria-label="Pre-sets de classe e traje"><header className="creator-panel-heading"><span>✦</span><div><p>PRE-SETS</p><small>CLASSE E TRAJE</small></div><span>✦</span></header><p className="creator-panel-note mt-4">Escolha um conjunto para equipar o aventureiro de uma vez.</p><div className="mt-4 grid grid-cols-2 gap-3">{presets.map((preset) => { const Icon = preset.icon; const active = activePreset === preset.id; return <button type="button" key={preset.id} onClick={() => selectPreset(preset)} className={`creator-preset ${active ? "creator-preset--active" : ""}`}><span className="creator-preset-icon"><Icon className="h-7 w-7" /></span><span className="mt-2 block text-[10px] font-black uppercase tracking-wide">{preset.label}</span><span className="mt-1 block text-[8px] leading-tight opacity-75">{preset.helper}</span></button>; })}</div><div className="mt-6 border-t-2 border-[#b29261] pt-5"><p className="creator-mini-heading">AURAS E FUNDOS</p><div className="mt-3 grid grid-cols-3 gap-2">{auras.map((option) => <button type="button" key={option.name} title={option.name} onClick={() => setAura(option)} className={`creator-aura-option ${aura.name === option.name ? "creator-aura-option--active" : ""}`}><span style={{ background: `radial-gradient(circle at 35% 30%,#fff7c9 0 8%,${option.color} 10% 46%,#4e432b 48% 55%,${option.color} 57% 69%,transparent 70%)` }} /></button>)}</div></div><div className="mt-5 creator-free-note">O pacote atual fornece 2 cabelos, 2 camisas e 1 acessório. Novas camadas surgem quando houver novos sprites.</div></section>

    <section className="creator-stage" aria-label="Pré-visualização do personagem"><div className="creator-stage-title"><span>✦</span> PREPARANDO O HERÓI <span>✦</span></div><div className="creator-arcane-aura" style={{ "--aura-wash": aura.wash } as React.CSSProperties} /><div className="creator-rune-circle"><span>ᚠ</span><span>ᚢ</span><span>ᚦ</span><span>ᚨ</span></div><div role="img" aria-label={`Prévia de ${name || "novo herói"}`} className="mana-seed-sprite creator-full-sprite"><ManaSeedSpriteLayers frame={MANA_SEED_FREE.staticAvatarFrame} layers={layers} /></div><div className="creator-stone-pedestal"><div className="creator-pedestal-runes">ᚠ&nbsp;&nbsp;ᚢ&nbsp;&nbsp;ᚦ&nbsp;&nbsp;ᚨ&nbsp;&nbsp;ᚱ&nbsp;&nbsp;ᚲ&nbsp;&nbsp;ᚷ</div><div className="creator-pedestal-plaque">{name || "HERÓI SEM NOME"}</div></div><div className="creator-stage-caption"><span style={{ backgroundColor: aura.color }} /> Aura: {aura.name} <b>·</b> {bodyType === "hero" ? "Herói" : "Heroína"}</div></section>
