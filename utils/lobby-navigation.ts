import { LOBBY_CORRIDORS, LOBBY_DESTINATIONS, LOBBY_MAP } from "@/data/lobby-map";
...
export function resolveMove(from: LobbyPoint, to: LobbyPoint): LobbyPoint {
  ...
  for (const corridor of LOBBY_CORRIDORS) {
    ...
    best = distance <= limit || distance === 0
      ? closest
      : { x: closest.x + ((to.x - closest.x) / distance) * limit, y: closest.y + ((to.y - closest.y) / distance) * limit };
  }
