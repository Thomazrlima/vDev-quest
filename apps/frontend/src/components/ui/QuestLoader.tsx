...
export function QuestLoader({ label = "Carregando...", hint, fullscreen = false, leaving = false, className = "" }: QuestLoaderProps) {
    return (
        <div aria-live="polite" className={`quest-loader ${fullscreen ? "quest-loader--fullscreen" : ""} ${leaving ? "quest-loader--leaving" : ""} ${className}`} role="status">
            <span className="quest-loader__stage">
                <span aria-hidden="true" className="quest-loader__halo" />
                <Logo className="quest-loader__mark" imageClassName="quest-loader__image" priority={fullscreen} size={256} />
            </span>
