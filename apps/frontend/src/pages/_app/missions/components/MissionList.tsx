...
export function MissionList({ missions, loading, onOpen, onInvalidate }: { missions: Mission[]; loading: boolean; onOpen: (mission: Mission) => void; onInvalidate?: (mission: Mission) => void }) {
    return (
                    ...
                    <article key={mission.id} className="grid grid-cols-[1fr_auto] items-center gap-4 border-b border-primary-dark px-5 py-5 transition odd:bg-black-soft even:bg-black-muted last:border-b-0 hover:bg-primary-overlay sm:grid-cols-[1.4fr_.7fr_.7fr_auto] sm:px-7">
                        ...
                        <div className="flex flex-wrap justify-end gap-2">
                            <Button variant="secondary" onClick={() => onOpen(mission)} className="border-2 border-primary-dark px-3 py-2 text-[10px] text-primary-light">
                                {mission.hasProgress || mission.status === "Invalidada" ? "Ver" : "Editar"}
                            </Button>
                            {mission.status === "Ativa" && onInvalidate ? <Button variant="secondary" onClick={() => onInvalidate(mission)} className="border-2 border-red px-3 py-2 text-[10px] text-red-light">Invalidar</Button> : null}
                        </div>
                    </article>
                ))
            )}
