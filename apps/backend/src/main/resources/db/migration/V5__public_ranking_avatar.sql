  ...
  select
    ...
    coalesce((
      select jsonb_build_object(
        'bodyType', a.body_type::text,
        'skinColorIndex', a.skin_color_index,
        'slots', coalesce((
          select jsonb_agg(jsonb_build_object('slot', setting.slot::text, 'code', item.code, 'colorIndex', setting.color_index) order by definition.draw_order)
          ...
          where setting.user_email = u.email
        ), '[]'::jsonb)
      )
      from avatar.user_avatars a where a.user_email = u.email
    ), jsonb_build_object('bodyType', 'hero', 'skinColorIndex', 0, 'slots', '[]'::jsonb)) as avatar
  from core.users u
