...
create policy users_self_or_manager on core.users using (email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'));
create policy missions_read_active_or_manager on quests.missions for select using (
  status = 'active'
  or (select current_setting('app.user_role', true) = 'manager')
  or exists (
    select 1 from quests.submissions s
    where s.mission_id = quests.missions.id
      and s.collaborator_email = (select nullif(current_setting('app.user_email', true), ''))
  )
);
create policy missions_manager_write on quests.missions for all using ((select current_setting('app.user_role', true) = 'manager')) with check ((select current_setting('app.user_role', true) = 'manager'));
...
create policy phases_manager_write on quests.mission_phases for all using ((select current_setting('app.user_role', true) = 'manager')) with check ((select current_setting('app.user_role', true) = 'manager'));
...
create policy weekdays_manager_write on quests.mission_weekdays for all using ((select current_setting('app.user_role', true) = 'manager')) with check ((select current_setting('app.user_role', true) = 'manager'));
create policy submissions_self_or_manager on quests.submissions using (collaborator_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager')) with check (collaborator_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'));
create policy evidence_owner_or_manager on quests.submission_phase_evidences using (exists (select 1 from quests.submissions s where s.id = submission_id and (s.collaborator_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager')))) with check (exists (select 1 from quests.submissions s where s.id = submission_id and (s.collaborator_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'))));
create policy avatar_self on avatar.user_avatars using (user_email = (select nullif(current_setting('app.user_email', true), ''))) with check (user_email = (select nullif(current_setting('app.user_email', true), '')));
create policy avatar_slots_self on avatar.user_slot_settings using (user_email = (select nullif(current_setting('app.user_email', true), ''))) with check (user_email = (select nullif(current_setting('app.user_email', true), '')));
create policy avatar_manager_read on avatar.user_avatars for select using ((select current_setting('app.user_role', true) = 'manager'));
create policy avatar_slots_manager_read on avatar.user_slot_settings for select using ((select current_setting('app.user_role', true) = 'manager'));
create policy awards_self_or_manager on gamification.xp_awards using (user_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager')) with check (user_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'));
create policy badges_self_or_manager on gamification.user_badges using (user_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'));
create policy titles_self_or_manager on gamification.user_titles using (user_email = (select nullif(current_setting('app.user_email', true), '')) or (select current_setting('app.user_role', true) = 'manager'));
create policy idempotency_self on private.idempotency_requests using (actor_email = (select nullif(current_setting('app.user_email', true), ''))) with check (actor_email = (select nullif(current_setting('app.user_email', true), '')));
