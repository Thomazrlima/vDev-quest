...
function initialSpawn() {
    ...
    return {
        position: { ...(exit ? exit.arrival : LOBBY_MAP.spawn) },
        pose: toIdlePose(exit ? "down" : LOBBY_MAP.spawnDirection),
    };
}
...
export function LobbyMap({ onReady }: { onReady?: () => void }) {
    ...
    const positionRef = useRef<LobbyPoint>(spawn.position);
    const cameraRef = useRef<LobbyPoint>({ x: 0, y: 0 });
    const viewportSizeRef = useRef({ width: 0, height: 0 });
    const poseRef = useRef(pose);
    const pressedKeysRef = useRef(new Set<string>());
    const activeHrefRef = useRef<LobbyHref | null>(activeHref);
     ...
     */
    const steeringVector = useCallback((position: LobbyPoint): LobbyPoint => {
    ...
    const cameraTarget = useCallback((position: LobbyPoint): LobbyPoint => {
        ...
        return {
            x: rangeX <= 0 ? rangeX / 2 : clamp(position.x * LOBBY_MAP.zoom - width / 2, 0, rangeX),
            y: rangeY <= 0 ? rangeY / 2 : clamp(position.y * LOBBY_MAP.zoom - height / 2, 0, rangeY),
        };
    }, []);
    ...
    const toWorldPoint = useCallback((clientX: number, clientY: number): LobbyPoint | null => {
    ...
    const handlePointerMove = useCallback(
        (event: ReactPointerEvent) => {
            ...
            joystickVectorRef.current = length > 0 ? { x: dx / length, y: dy / length } : null;
    ...
    // Câmera enquadrada antes da primeira pintura: senão o vilarejo aparece pelo
    // canto e desliza até o jogador durante o primeiro quadro.
    useLayoutEffect(() => {
        const viewport = viewportRef.current;
    ...
    useEffect(() => {
        ...
        function tick(time: number) {
            ...
            const vector = routingRef.current ? { x: 0, y: 0 } : steeringVector(positionRef.current);
            const previous = positionRef.current;

            if (vector.x || vector.y) {
                const distance = LOBBY_MAP.playerSpeed * deltaSeconds;
                positionRef.current = resolveMove(previous, { x: previous.x + vector.x * distance, y: previous.y + vector.y * distance });
            }
            ...
            if (moving) {
                ...
                if (activeHrefRef.current !== nextHref) {
                    ...
                    if (nextHref) void router.preloadRoute({ to: nextHref });
                }
            }
    ...
    }, [clearPath, enterDestination]);

    const stageStyle = { "--lobby-world": `${WORLD_SIZE}px`, "--lobby-sprite": `${SPRITE_SIZE}px` } as CSSProperties;

    return (
        <div className="lobby__viewport" ref={viewportRef} role="application" aria-label="Vilarejo navegável" onPointerCancel={handlePointerCancel} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} style={stageStyle}>
                ...
                {debug ? (
                    <div className="lobby__debug" aria-hidden="true">
                        {LOBBY_CORRIDORS.map((corridor) => {
                            const length = Math.hypot(corridor.b.x - corridor.a.x, corridor.b.y - corridor.a.y) * LOBBY_MAP.zoom;
                            const angle = (Math.atan2(corridor.b.y - corridor.a.y, corridor.b.x - corridor.a.x) * 180) / Math.PI;
            ...
            {transitionHref ? <span aria-hidden="true" className="lobby__fade" /> : null}
        </div>
    );
}
