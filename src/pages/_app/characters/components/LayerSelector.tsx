    ...
    return (
        <div className={`flex min-h-[54px] items-center gap-[.45rem] border-2 bg-[var(--color-black)] p-[.4rem] shadow-[inset_2px_2px_0_var(--color-primary-overlay),2px_2px_0_var(--color-black)] ${enabled ? "border-[var(--color-orange)] bg-[var(--color-orange-dark)]" : "border-[var(--color-orange-dark)]"}`}>
            ...
            <span className="min-w-0 flex-1">
                <span className="block text-[9px] font-black uppercase tracking-[.14em] text-[var(--color-orange)]">{renderTextWithNumericFont(label)}</span>
                <strong className="mt-1 block truncate text-xs text-[var(--color-orange-light)]">{renderTextWithNumericFont(value)}</strong>
            </span>
            ...
            <span className="w-9 text-center text-[9px] font-black text-[var(--color-orange)]">{renderTextWithNumericFont(count)}</span>
