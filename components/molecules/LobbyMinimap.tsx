...
import { LOBBY_DESTINATIONS, LOBBY_MAP } from "@/data/lobby-map";
...
export function LobbyMinimap({ dotRef }: { dotRef: RefObject<HTMLSpanElement | null> }) {
  return (
    <div className="lobby-minimap" aria-hidden="true" style={{ width: `${MINIMAP_SIZE}px`, height: `${MINIMAP_SIZE}px` }}>
      <Image
        alt=""
        className="lobby-minimap__image"
      ...
      {LOBBY_DESTINATIONS.map((destination) => (
        <span
          className="lobby-minimap__pin"
          key={destination.href}
          style={{ left: `${destination.arrival.x * MINIMAP_SCALE}px`, top: `${destination.arrival.y * MINIMAP_SCALE}px` }}
        />
      ))}
