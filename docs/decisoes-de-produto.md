...
| Acesso | O MVP utiliza SSO corporativo, criando o usuário no primeiro acesso com papel `collaborator`; o papel administrativo é alterado diretamente no banco. | O e-mail normalizado permanece a identidade canônica. O nome do usuário pode ser alterado no sistema e não é sincronizado a partir do SSO. |
